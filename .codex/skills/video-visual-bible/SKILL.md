...
Codex can read images directly via the Read tool (PNG, JPG, etc.). Use this built-in vision capability instead of external vision APIs.

After a key image is accepted:

1. **Read the accepted image** with Codex's Read tool — this provides visual understanding directly.
...
For multi-shot consistency checking or batch review of generated images, delegate to the `visual-reviewer` agent. This agent uses Codex's vision to:
...
- Codex's built-in vision is the primary image analysis tool — no external vision API required.
