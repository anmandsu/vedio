...
This project turns Codex into a lightweight AI film crew for script-to-video work. Treat video generation as film pre-production plus controlled execution, not as prompt writing.
...
5. **Visual bible**: lock characters, scenes, props, moodboards, visual fingerprints, and non-negotiable continuity anchors. Codex's built-in vision handles all image analysis — no external vision API needed.
...
Use `.Codex/agents/*` as the project-level temporary crew. Delegate large research or department review tasks to the relevant subagent when the output would otherwise flood the main context.
...
8. `visual-reviewer` — reviews generated images using Codex's built-in vision. Checks 10 continuity layers against the visual bible, produces structured review reports with BLOCK/WARNING/INFO severity, extracts visual fingerprints from approved frames.
...
Read `.Codex/references/knowledge-source-policy.md` before using external film knowledge. Every borrowed idea must be traceable to a source tier:
...
Use `.Codex/scripts/new_video_project.py` to initialize this structure.
...
| VPipe YAML validates | `python .Codex/scripts/validate_vpipe_yaml.py <file>` exits 0 | Validation errors present |
...
**Generation strategy:** Prefer reference-image and fingerprint-driven prompts over long abstract prompts. Moodboards and visual fingerprints are stronger than adjectives. Use Codex's built-in vision for all image analysis.
...
See `.Codex/skills/scene-prompt-writer/SKILL.md` for detailed workflow, research strategies, and generation commands.
