...
Read `.claude/references/shot-schema-vpipe.md` before writing or editing VPipe YAML.
...
For each reference case, read the output image (`images/<category>_case<N>/output.jpg`) with Claude Code's Read tool to understand what the prompt actually produces.
