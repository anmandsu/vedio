...
**Concurrent generation:** External image generation can use the async LinkAPI CLI in `.claude/scripts/gen_api.py`. For batch jobs, prefer one shared `aiohttp.ClientSession` through:

```bash
python .claude/scripts/gen_api.py batch "<manifest.json>" --output-dir="<renders_dir>" --concurrency=10 --download --download-concurrency=10
```
